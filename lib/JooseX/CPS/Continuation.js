...
//            if (!this.tryScope && parent.tryScope) 
        ...
        setTry : function (tryFunc, tryScope, tryArgs) {
            ...
            var length = this.nested.length
            var previousScope = length && this.nested[ length - 1 ].scope
            
            this.nested.push({
                func    : tryFunc, 
                scope   : tryScope || this.tryScope || Joose.top, //explicitly passed scope has the highest priority  
                args    : tryArgs
            })
        ...
        start : function () {
            ...
            setTimeout(function() {
                ...
                JooseX.CPS.Continuation.my.CONT = scope.CONT = cont
                
                try {
                    if (current.func.apply(scope, current.args || results) !== undefined) throw "ERROR: Value returned from continued function (use `CONTINUE(value)` instead)" 
                } catch (e) {
                    ...
                    JooseX.CPS.Continuation.my.CONT = null
        ...
        unnest : function (returned, values, isReturn) {
            if (returned == this.nextCont || (!this.nextCont && !this.nested.length)) {
                //moving back                
    ...
    my : {
        have : {
            // Global reference to current continuation
            // Is valid only inside of continued scope
            CONT    : null
        }
    }
})